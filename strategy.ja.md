...
```mermaid
flowchart TB
    S1["<b>1. 探索・情報収集</b><br/>セキュア環境内<br/>Aider /ask + Qwen 3.5<br/>スキーマ、ファイル構成、カラム名、<br/>ツールバージョン、エラーメッセージを収集"]:::inside
    S2["<b>2. Claudeで計画</b><br/>外部PC<br/>メタデータ＋要件を渡し<br/>分析計画・実装指示を受け取る"]:::outside
...
flowchart TB
    subgraph boundary["セキュアネットワーク境界"]
        ...
        subgraph model["モデルサーバー（Ollama）"]
            qwen["Qwen 3.5 (9B-72B)<br/>GPU or CPUメモリにロード"]
        end
        aider -->|"API"| qwen
    end
